...
async function generatePPTX() {
    console.log('--- PPTX Generation Started ---');
    ...
    try {
        ...
        slide1.addText('DANKO VPP v2.0', { x: 0, y: 1.5, w: '100%', h: 1, fontSize: 44, bold: true, color: '1E293B', align: 'center' });
        slide1.addText('HỆ THỐNG QUẢN TRỊ CUNG ỨNG NỘI BỘ', { x: 0, y: 2.5, w: '100%', h: 0.5, fontSize: 20, color: '64748B', align: 'center' });
        slide1.addText('HƯỚNG DẪN VẬN HÀNH CHI TIẾT', { x: 0, y: 4.5, w: '100%', h: 0.5, fontSize: 24, bold: true, color: '4F46E5', align: 'center' });
        console.log('Slide 1 added');
    } catch(e) { console.error('Error Slide 1:', e); }

    // Helper for content slides
    const addContentSlide = (title, description, imageName) => {
        try {
            console.log(`Adding slide: ${title}`);
            let slide = pptx.addSlide();
            slide.addText(title, { x: 0.4, y: 0.2, w: 9, h: 0.4, fontSize: 28, bold: true, color: '1E293B' });
            slide.addText(description, { x: 0.4, y: 0.8, w: 3, h: 4, fontSize: 13, color: '475569', bullet: true, valign: 'top' });
            
            if (imageName) {
                const fullPath = path.join(screenshotsDir, imageName);
        ...
        } catch(e) { console.error(`Error adding slide ${title}:`, e); }
    };
    ...
    try {
        ...
        slideEnd.addText('QUY TẮC VẬN HÀNH VÀNG', { x: 0, y: 1, w: '100%', h: 1, fontSize: 36, bold: true, color: 'FFFFFF', align: 'center' });
        slideEnd.addText('1. Không giao hàng khi chưa có lệnh Issue trên hệ thống.\n2. Nhân viên bắt buộc bấm Xác Nhận Nhận Hàng để đóng phiếu.\n3. Quản lý duyệt phiếu trong vòng 4h kể từ khi nhận thông báo.\n4. Mọi sai sót số liệu cần báo ngay cho Admin để điều chỉnh Audit.', { 
            x: 1, y: 2.5, w: 8, h: 3, fontSize: 18, color: 'E2E8F0', align: 'left', bullet: true 
        });
        console.log('Final slide added');
    } catch(e) { console.error('Error Final Slide:', e); }

    try {
        const outPath = path.join(process.cwd(), 'Danko_VPP_Manual_Final.pptx');
        await pptx.writeFile({ fileName: outPath });
        console.log(`--- PPTX SUCCESS: ${outPath} ---`);
    } catch (err) {
