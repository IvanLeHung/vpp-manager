...
export default function GuestRequest() {
  ...
  useEffect(() => {
    ...
    api.get('/public/departments')
      .then(res => setDepts(Array.isArray(res.data) ? res.data : (res.data?.data || [])))
      .catch(console.error);
  }, []);
  ...
  const submitForm = async () => {
    ...
    if (!deptId) return alert('Vui lòng chọn Bộ phận công tác!');
    if (targetItems.length === 0) return alert('Chưa chọn mặt hàng VPP nào.');

    try {
      setLoading(true);
      const selectedDept = depts.find(d => d.id === deptId);
      const phoneInfo = guestPhone ? `${guestPhone} - ${selectedDept?.name}` : selectedDept?.name;

      await api.post('/public/requests', {
        guestName,
        guestPhone: phoneInfo,
        purpose,
      ...
      <div className="w-full max-w-4xl bg-white rounded-3xl shadow-xl overflow-hidden">
        ...
        <div className="p-8">
          ...
          <div className="mb-8 p-6 bg-slate-50 border border-slate-200 rounded-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                 ...
                 <input type="text" value={guestPhone} onChange={e=>setGuestPhone(e.target.value)} placeholder="Nhập SĐT (không bắt buộc)..." className="w-full p-3 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-medium transition" />
              </div>
              <div>
                 <label className="block text-xs font-bold text-slate-700 mb-1.5">Bộ phận công tác <span className="text-rose-500">*</span></label>
                 <select 
                    value={deptId} 
                    onChange={e=>setDeptId(e.target.value)} 
                    className="w-full p-3 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-medium transition"
                 >
          ...
          <button 
            disabled={loading}
            onClick={submitForm} 
            className="w-full flex justify-center items-center py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-lg shadow-lg hover:shadow-indigo-500/50 transition-all duration-300 cursor-pointer disabled:opacity-50"
          >
            <Send className="w-5 h-5 mr-2" /> 
            {loading ? 'Đang gửi yêu cầu...' : 'Gửi Yêu Cầu Cấp Phát VPP'}
          </button>
        </div>
      </div>
    </div>
