...
const STATUS_CONFIG: Record<string, { label: string; cls: string; bgCls: string; icon: any }> = {
  ...
  WAITING_HANDOVER: { label: 'Chờ bàn giao', cls: 'text-blue-600', bgCls: 'bg-blue-50 border-blue-200', icon: Clock },
  HANDED_OVER: { label: 'Hoàn thành', cls: 'text-indigo-700', bgCls: 'bg-indigo-50 border-indigo-300', icon: CheckCircle2 },
  CANCELLED: { label: 'Đã hủy', cls: 'text-gray-500', bgCls: 'bg-gray-100 border-gray-300', icon: Ban },
  RETURNED: { label: 'Đã hoàn kho', cls: 'text-rose-600', bgCls: 'bg-rose-50 border-rose-200', icon: ArrowLeft },
};
...
const AUDIT_LABELS: Record<string, string> = {
  ...
  HANDOVER: 'Bàn giao', RETURN: 'Hoàn kho',
};
...
export default function WarehouseTicketDetail({ basePath = '/warehouse-tickets' }: { basePath?: string }) {
  ...
  const handleHandover = async () => {
    try {
      setActionLoading(true);
      await api.post(`/warehouse-tickets/${id}/handover`);
      addToast('Xác nhận bàn giao thành công!');
      fetchTicket();
    } catch (err: any) {
      addToast('Lỗi: ' + (err.response?.data?.error || 'Không thể xác nhận bàn giao'), 'error');
    } finally { setActionLoading(false); }
  };
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] p-4 md:p-8 bg-slate-50 overflow-auto relative print:p-0 print:overflow-visible print:h-auto print:bg-white print:block">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 no-print shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1">
              ...
              {ticket.sourceRef && (
                <span className="text-sm text-indigo-600 font-bold bg-indigo-50 px-2 py-0.5 rounded-lg border border-indigo-100 flex items-center">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button onClick={() => window.print()} className="flex items-center px-4 py-2.5 bg-white text-indigo-600 border-2 border-indigo-200 rounded-xl font-black hover:bg-indigo-50 shadow-md transition transform hover:scale-[1.02]">
             <ClipboardList className="w-5 h-5 mr-2" /> IN CHỨNG TỪ KHO
          </button>
          ...
          {/* HANDOVER: Executed/Waiting -> Handed Over */}
          {(isAdmin || isWarehouse) && ['EXECUTED', 'WAITING_HANDOVER'].includes(ticket.status) && ticket.ticketType === 'ISSUE' && (
             <button onClick={handleHandover} disabled={actionLoading} className="flex items-center px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 shadow-lg transition disabled:opacity-50">
               <CheckCircle2 className="w-4 h-4 mr-2" /> Xác nhận bàn giao
             </button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 no-print shrink-0">
                  ...
                  {ticket.lines.map((line, i) => (
                    <tr key={line.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="p-4 text-right font-bold text-slate-700">
                        {line.unitPrice ? `${Number(line.unitPrice).toLocaleString('vi-VN')} đ` : '—'}
                      </td>
                      <td className="p-4 text-right font-black text-indigo-600">
                        {line.totalAmount ? `${Number(line.totalAmount).toLocaleString('vi-VN')} đ` : '—'}
                      </td>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            ...
            <div className="space-y-4 text-sm">
              <div className="flex items-center">
                ...
                <span className="font-bold text-slate-700">{ticket.createdBy.fullName} <span className="text-xs text-slate-400">({ROLE_LABELS[ticket.createdBy.role] || ticket.createdBy.role})</span></span>
              </div>
              <div className="flex items-center">
          ...
          {/* Receiver Info Card */}
          {ticket.receiverName && (
            <div className="bg-indigo-50/50 rounded-2xl shadow-sm border border-indigo-100 p-6">
              <h3 className="font-bold text-indigo-900 mb-4 flex items-center gap-2">
                <User className="w-4 h-4" /> Thông tin tiếp nhận
                <span className="ml-auto px-2 py-0.5 bg-indigo-100 text-indigo-600 rounded text-[10px] font-black uppercase tracking-widest">{ticket.itemGroup || 'VPP'}</span>
              </h3>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between"><span className="text-slate-500">Đối tượng:</span> <span className="font-bold text-slate-800 italic uppercase text-[10px]">{ticket.receiverType || 'PHONG_BAN'}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Người nhận/Quản lý:</span> <span className="font-bold text-slate-800">{ticket.receiverName}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Bộ phận/Đơn vị:</span> <span className="font-bold text-slate-800">{ticket.receiverDept}</span></div>
                {ticket.receiverAddress && <div className="flex justify-between"><span className="text-slate-500">Khu vực/Địa chỉ:</span> <span className="font-bold text-slate-800">{ticket.receiverAddress}</span></div>}
                
                {/* Metadata Fields */}
                {ticket.metadata?.period && <div className="flex justify-between"><span className="text-emerald-600 font-bold">Chu kỳ cấp phát:</span> <span className="font-black text-emerald-700">{ticket.metadata.period}</span></div>}
                {ticket.metadata?.quota && <div className="flex justify-between"><span className="text-emerald-600 font-bold">Định mức:</span> <span className="font-black text-emerald-700">{ticket.metadata.quota}</span></div>}
                {ticket.metadata?.assetCode && <div className="flex justify-between"><span className="text-amber-600 font-bold">Mã tài sản:</span> <span className="font-black text-amber-700">{ticket.metadata.assetCode}</span></div>}
                {ticket.metadata?.handoverDate && <div className="flex justify-between"><span className="text-amber-600 font-bold">Ngày bàn giao:</span> <span className="font-black text-amber-700">{new Date(ticket.metadata.handoverDate).toLocaleDateString('vi-VN')}</span></div>}

                <div className="pt-2 border-t border-indigo-100 flex justify-between">
                  <span className="text-slate-500">Nghiệp vụ:</span> 
                  <span className="px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded-md font-bold text-[10px] uppercase tracking-widest">
                ...
                {ticket.auditTrail.map((a, i) => (
                  <div key={a.id} className="flex gap-3 text-sm">
                    ...
                    <div className="pb-3">
                      ...
                      <p className="text-slate-400 text-xs font-medium">{a.user?.fullName || 'Hệ thống'} • {new Date(a.createdAt).toLocaleString('vi-VN')}</p>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  <tbody>
                      {ticket.lines.map((line, idx) => (
                          <tr key={line.id} className="h-10">
                              ...
                              <td className="border border-black p-2 text-right">{(line.unitPrice || 0).toLocaleString('vi-VN')}₫</td>
                              <td className="border border-black p-2 text-right font-black">{(line.totalAmount || 0).toLocaleString('vi-VN')}₫</td>
                          </tr>
                      ))}
                      <tr className="bg-slate-50 h-10 font-black">
                          ...
                          <td className="border border-black p-2 text-right">
                              {(ticket.totalAmount || 0).toLocaleString('vi-VN')}₫
                          </td>
