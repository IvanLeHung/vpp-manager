...
export default function Analytics() {
  ...
  const fetchData = async (currentFilters = filters) => {
    try {
      ...
      // Individual requests to prevent one failure from blocking all
      try {
        const res = await api.get(`/reports/dashboard?${qs}`);
        setKpis(res.data);
      } catch (e) { console.error('Failed to load KPIs', e); }

      try {
        const res = await api.get(`/reports/inventory-flow?${qs}`);
        setInventoryFlow(res.data || []);
      } catch (e) { console.error('Failed to load inventory flow', e); }

      try {
        const res = await api.get(`/reports/departmental-usage?${qs}`);
        setDeptUsage(res.data || []);
      } catch (e) { console.error('Failed to load dept usage', e); }

      try {
        const res = await api.get(`/reports/advanced-metrics?${qs}`);
        setAdvanced(res.data);
      } catch (e) { console.error('Failed to load advanced metrics', e); }

      try {
        const res = await api.get(`/reports/departmental-behavior`);
        setBehavior(res.data || []);
      } catch (e) { console.error('Failed to load behavior', e); }

      try {
        const res = await api.get(`/reports/fulfillment-analysis?${qs}`);
        setFulfillment(res.data);
      } catch (e) { console.error('Failed to load fulfillment analysis', e); }
  ...
  return (
    <div className="flex flex-col min-h-full p-4 md:p-10 bg-slate-50 relative overflow-y-auto custom-scrollbar">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8 mb-12">
            ...
            />
            <StatCard 
              label="Khả Năng Đáp Ứng Yêu Cầu" 
              value={fulfillment ? `${fulfillment.summary.rate}%` : '0%'} 
              icon={<TrendingUp className="w-6 h-6"/>}
              trend={fulfillment?.summary.status || 'Ổn định'}
              subtext={fulfillment ? `Thiếu ${fulfillment.summary.missing} / ${fulfillment.summary.totalRequested} đơn vị` : ''}
              color={fulfillment?.summary.rate < 50 ? 'rose' : (fulfillment?.summary.rate < 80 ? 'amber' : 'emerald')}
            />
            <StatCard 
              label="Giá Trị Tồn Kho" 
              value={formatCurrency(inventoryFlow.reduce((s, i) => s + i.value, 0))} 
              icon={<Package className="w-6 h-6"/>}
              trend={inventoryFlow.some(i => i.closing < 0) ? 'Bất thường' : 'Ổn định'}
              color={inventoryFlow.some(i => i.closing < 0) ? 'rose' : 'blue'}
            />
        </div>
        ...
        {/* ========================================== */}
        {/* ROW 5: DEPARTMENTAL BEHAVIOR ANALYSIS */}
        {/* ========================================== */}
        <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden mb-12 no-print">
            <div className="p-10 border-b border-slate-50 bg-indigo-50/20 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h3 className="text-2xl font-black text-slate-800 tracking-tighter uppercase italic flex items-center gap-3">
                        <Activity className="w-8 h-8 text-indigo-500" /> Phân Tích Hành Vi Đề Xuất (Decision Support)
                    </h3>
                    <p className="text-sm font-bold text-slate-400 mt-1 italic uppercase tracking-widest">Phát hiện bất thường và xu hướng tiêu dùng theo phòng ban</p>
                </div>
            </div>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left">
                    <thead className="bg-slate-50/50 border-b border-slate-100">
                        <tr className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] italic">
                            <th className="p-8">Phòng ban</th>
                            <th className="p-8">Phiếu gần nhất</th>
                            <th className="p-8">Phiếu trước đó</th>
                            <th className="p-8 text-center">Khoảng cách (Ngày)</th>
                            <th className="p-8 text-center">Biến động (%)</th>
                            <th className="p-8 text-right">Trạng thái rủi ro</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                        {behavior.map((d: any) => (
                            <tr key={d.department} className="hover:bg-slate-50/50 transition-colors">
                                <td className="p-8">
                                    <span className="text-sm font-black text-slate-800 uppercase tracking-tighter">{d.department}</span>
                                </td>
                                <td className="p-8">
                                    <div className="flex flex-col">
                                        <span className="text-xs font-bold text-indigo-600">{d.latestRequest.id}</span>
                                        <span className="text-[10px] text-slate-400">{new Date(d.latestRequest.date).toLocaleDateString('vi-VN')} • {d.latestRequest.qty} món</span>
                                    </div>
                                </td>
                                <td className="p-8">
                                    {d.previousRequest ? (
                                        <div className="flex flex-col">
                                            <span className="text-xs font-bold text-slate-500">{d.previousRequest.id}</span>
                                            <span className="text-[10px] text-slate-400">{new Date(d.previousRequest.date).toLocaleDateString('vi-VN')} • {d.previousRequest.qty} món</span>
                                        </div>
                                    ) : <span className="text-[10px] text-slate-300 italic">N/A</span>}
                                </td>
                                <td className="p-8 text-center">
                                    <span className={`text-sm font-black ${d.gapDays !== null && d.gapDays < 7 ? 'text-rose-500 animate-pulse' : 'text-slate-700'}`}>
                                        {d.gapDays ?? '--'}
                                    </span>
                                </td>
                                <td className="p-8 text-center">
                                    <span className={`text-sm font-black ${d.growth > 50 ? 'text-rose-500' : (d.growth < 0 ? 'text-emerald-500' : 'text-slate-700')}`}>
                                        {d.growth > 0 ? `+${Math.round(d.growth)}%` : `${Math.round(d.growth)}%`}
                                    </span>
                                </td>
                                <td className="p-8 text-right">
                                    {d.isSuspicious ? (
                                        <span className="bg-rose-100 text-rose-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border border-rose-200">Rủi ro cao</span>
                                    ) : (
                                        <span className="bg-emerald-100 text-emerald-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border border-emerald-200">Bình thường</span>
                                    )}
                                </td>
                            </tr>
                        ))}
        ...
        {/* ========================================== */}
        {/* ROW 6: FULFILLMENT ANALYSIS & RESTOCK SUGGESTIONS */}
        {/* ========================================== */}
        <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden mb-20 no-print">
            <div className="p-10 border-b border-slate-50 bg-rose-50/20 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h3 className="text-2xl font-black text-slate-800 tracking-tighter uppercase italic flex items-center gap-3">
                        <AlertTriangle className="w-8 h-8 text-rose-500" /> Vật Tư Cần Bổ Sung (Dựa trên phiếu đang chờ)
                    </h3>
                    <p className="text-sm font-bold text-slate-400 mt-1 italic uppercase tracking-widest">Danh sách vật tư không đủ tồn kho để đáp ứng toàn bộ yêu cầu hiện tại</p>
                </div>
                <div className="flex items-center gap-4 text-xs font-black uppercase">
                    <div className="px-4 py-2 bg-rose-100 text-rose-700 rounded-xl">Thiếu: {fulfillment?.summary.missing || 0} đơn vị</div>
                    <div className="px-4 py-2 bg-slate-100 text-slate-700 rounded-xl">Phiếu đáp ứng đủ: {fulfillment?.requestStats.fullyFulfillable}/{fulfillment?.requestStats.total}</div>
                </div>
            </div>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left">
                    <thead className="bg-slate-50/50 border-b border-slate-100">
                        <tr className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] italic">
                            <th className="p-8">Vật tư</th>
                            <th className="p-8 text-center">ĐVT</th>
                            <th className="p-8 text-center">Tổng yêu cầu</th>
                            <th className="p-8 text-center">Tồn khả dụng</th>
                            <th className="p-8 text-center text-rose-600">Số lượng thiếu</th>
                            <th className="p-8 text-right text-indigo-600">Đề xuất nhập</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                        {fulfillment?.missingItems.length === 0 ? (
                            <tr><td colSpan={6} className="p-20 text-center text-slate-300 font-black italic uppercase tracking-widest">Tồn kho hiện tại đã đáp ứng tốt mọi yêu cầu chờ xử lý</td></tr>
                        ) : fulfillment?.missingItems.map((item: any) => (
                            <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                                <td className="p-8">
                                    <div className="flex flex-col">
                                        <span className="text-sm font-black text-slate-800 uppercase tracking-tighter">{item.name}</span>
                                        <span className="text-[10px] font-bold text-slate-400 tracking-widest">{item.mvpp}</span>
                                    </div>
                                </td>
                                <td className="p-8 text-center text-xs font-bold text-slate-500 uppercase">{item.unit}</td>
                                <td className="p-8 text-center text-sm font-black text-slate-800 tabular-nums">{item.qtyRequested}</td>
                                <td className="p-8 text-center text-sm font-bold text-slate-400 tabular-nums">{item.available}</td>
                                <td className="p-8 text-center text-base font-black text-rose-600 tabular-nums">{item.missing}</td>
                                <td className="p-8 text-right">
                                    <div className="flex flex-col items-end">
                                        <span className="text-lg font-black text-indigo-700 tabular-nums">{item.suggestedPurchase}</span>
                                        <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Min recommendation</span>
                                    </div>
                                </td>
                            </tr>
                        ))}
...
function StatCard({ label, value, icon, trend, subtext, color }: any) {
  ...
  return (
    <div className="bg-white rounded-[2.5rem] p-10 shadow-xl border border-slate-100 relative group overflow-hidden hover:transform hover:-translate-y-2 transition-all duration-500">
      ...
      <h3 className="text-3xl font-black text-slate-900 tracking-tighter italic leading-none truncate mb-1" title={String(value)}>
        {value}
      </h3>
      {subtext && <p className="text-[10px] font-bold text-slate-400 italic truncate">{subtext}</p>}
    </div>
  );
}
