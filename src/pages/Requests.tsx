...
export default function RequestsCreate({
  ...
  useEffect(() => {
    ...
    const hydrateLine = async (line: any): Promise<TargetItem> => {
      // Ưu tiên line.item nếu request detail đã có sẵn dữ liệu
      if (line.item && (line.item.name || line.item.mvpp)) {
        return {
          itemId: line.itemId,
      ...
      // Fallback gọi API item detail
      try {
        const itemRes = await api.get(`/items/${line.itemId}`);
        const itemData = itemRes.data?.data ?? itemRes.data;
        return {
          itemId: line.itemId,
          item: normalizeHydratedItem(itemData, line),
          quantity: Number(line.qtyRequested || 1),
          note: line.note || '',
        };
      } catch {
    ...
    const hydrateDraft = async () => {
      ...
      let sourceRequest: any = activeRequest;

      // Luôn cố gắng lấy detail mới nhất để giữ nguyên list item
      try {
        const detailRes = await api.get(`/requests/${activeRequest.id}`);
        sourceRequest = detailRes.data?.data ?? detailRes.data ?? activeRequest;
      } catch {
        sourceRequest = activeRequest;
      }
      ...
      const prefilled = await Promise.all(
        (sourceRequest.lines || []).map((line: any) => hydrateLine(line))
      );
