...
export default function ImportExcelModal({
  ...
  const resetState = () => {
    setStep(1);
    setMode('ADD_AND_UPDATE');
    setFile(null);
    setParsedData([]);
    setStats({ total: 0, valid: 0, error: 0, new: 0, update: 0 });
    setLoading(false);
  ...
  const handleConfirm = async () => {
    ...
    const payloadItems = parsedData.map((row) => ({
      action:
        row.errors?.length > 0
          ? 'IGNORE'
          : mode === 'ADD_ONLY'
    ...
    try {
      const resp = await api.post<ImportConfirmResponse>('/items/import/confirm', {
        mode,
      ...
      const batch = resp.data?.data?.batch;

      if (!summary) {
        alert(resp.data?.message || 'Import hoàn tất.');
        onSuccess();
        handleClose();
        return;
      }

      if (summary.successRows > 0 && summary.errorRows === 0) {
        alert(`Import thành công ${summary.successRows} mặt hàng!`);
        onSuccess();
        handleClose();
        return;
      }
      ...
      if (summary.successRows > 0 && summary.errorRows > 0) {
        alert(
          `Import hoàn tất: ${summary.successRows} thành công, ${summary.errorRows} lỗi.\nTrạng thái batch: ${
      ...
      alert(
        `Import thất bại: 0 thành công, ${summary.errorRows} lỗi.\nVui lòng mở "Lịch sử Import Excel" để xem chi tiết.`
      );
    } catch (err: any) {
      const responseData: ImportConfirmResponse | undefined = err.response?.data;
      const summary = responseData?.data?.summary;
      const batch = responseData?.data?.batch;

      if (summary) {
        alert(
          `Import thất bại: ${summary.successRows} thành công, ${summary.errorRows} lỗi.\n` +
            `Trạng thái batch: ${batch?.status || 'FAILED'}\n` +
            (responseData?.message || responseData?.error || 'Có lỗi khi import.')
        );
        return;
      }

      alert('Lỗi import: ' + (responseData?.error || err.message));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[90vh] border border-white/20 animate-in fade-in zoom-in duration-300">
        <div className="px-8 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
          ...
          <button
            onClick={handleClose}
            className="text-slate-400 hover:text-rose-500 bg-white p-2 border border-slate-200 rounded-full shadow-sm hover:bg-rose-50 transition"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="lg:col-span-8 space-y-6">
                <div className="bg-indigo-50 border border-indigo-100 p-6 rounded-2xl flex items-start gap-4">
                  <div className="w-10 h-10 bg-indigo-600 rounded-full flex items-center justify-center text-white shrink-0 font-bold">
                    1
                  </div>
                  <div className="flex-1">
                    <h3 className="font-bold text-indigo-900 mb-1">Cấu hình Import</h3>
                    <p className="text-sm text-indigo-600/80 mb-4 font-medium">
                      Chọn phương thức xử lý khi gặp mã hàng đã tồn tại trong hệ thống.
                    </p>
                    <select
                      value={mode}
                      onChange={(e) => setMode(e.target.value as ImportMode)}
                      className="w-full px-4 py-3 bg-white border-2 border-indigo-100 rounded-xl focus:border-indigo-500 outline-none font-bold text-indigo-800 transition shadow-sm"
                    >
                ...
                >
                  <FileSpreadsheet
                    className={`w-16 h-16 mx-auto mb-4 ${file ? 'text-emerald-500' : 'text-slate-300'}`}
                  />
                  {file ? (
                    <>
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-white rounded-xl border border-slate-200">
                    ...
                    <button
                      ...
                      className="w-full py-2.5 bg-indigo-50 text-indigo-700 text-sm font-bold rounded-lg hover:bg-indigo-100 transition flex items-center justify-center gap-2"
                    >
                  ...
                  <div className="p-4 bg-white rounded-xl border border-slate-200">
                    ...
                    <ul className="text-xs text-slate-500 space-y-2 leading-relaxed font-medium">
                      <li className="flex gap-2">
                        <span>•</span> <strong>Mã VT:</strong> Phải là mã duy nhất (VPP001, VS002...)
                      </li>
                      <li className="flex gap-2">
                        <span>•</span> <strong>Loại kho:</strong> Chỉ nhận giá trị <b>VPP</b> hoặc <b>VE_SINH</b>
                      </li>
                      <li className="flex gap-2">
                        <span>•</span> <strong>Đơn giá:</strong> Số nguyên không âm.
                      </li>
                    </ul>
                  </div>
                </div>
                ...
                <table className="w-full text-left text-sm whitespace-nowrap min-w-max">
                  ...
                  <tbody className="divide-y divide-slate-100 italic-last-step">
                    {parsedData.map((row, i) => (
                      <tr
                        key={i}
                        className={`hover:bg-slate-50/50 transition ${row.errors.length > 0 ? 'bg-rose-50/30' : ''}`}
                      >
                        ...
                        <td className="p-4 text-center">
                          <span
                            className={`px-2.5 py-1 text-[10px] font-black rounded-lg border shadow-sm
                              ${
                                row.action === 'CREATE'
                          ...
                          {row.errors.length > 0 ? (
                            <div className="flex flex-col gap-1">
                              {row.errors.map((e, idx) => (
                                <span
                                  key={idx}
                                  className="text-[10px] text-rose-600 font-bold bg-rose-50 px-1.5 py-0.5 rounded border border-rose-100 w-fit shrink-0 tracking-tight leading-none uppercase"
                                >
                                  ! {e}
                                </span>
                              ))}
                            ...
                            <div className="flex items-center text-emerald-600 font-black text-[10px] uppercase tracking-widest">
                              <CheckCircle className="w-3.5 h-3.5 mr-1.5" /> Dữ liệu Sạch
                            </div>
                          )}
                        </td>
                      </tr>
        ...
        <div className="px-8 py-5 bg-slate-50/50 border-t border-slate-100 flex justify-between shrink-0">
          {step === 1 ? (
            <>
              <button
                onClick={handleClose}
                className="px-6 py-3 text-sm font-bold text-slate-500 bg-white border border-slate-200 hover:bg-slate-50 rounded-2xl transition shadow-sm"
              >
                Thoát
              </button>
              ...
              <button
                ...
                className="px-8 py-3 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-2xl transition-all flex items-center shadow-lg shadow-indigo-100 transform active:scale-95"
              >
                {loading ? (
                  <>
            ...
            <>
              <button
                onClick={() => setStep(1)}
                className="px-6 py-3 text-sm font-bold text-slate-500 bg-white border border-slate-200 hover:bg-slate-50 rounded-2xl transition"
              >
                Quay lại chọn file
              </button>
              ...
              <div className="flex gap-4">
                <button
                  onClick={handleClose}
                  className="px-6 py-3 text-sm font-bold text-slate-400 bg-transparent hover:text-rose-600 transition"
                >
                  Huỷ bỏ cả Batch
                </button>
                ...
                <button
                  ...
                  disabled={(mode !== 'VALIDATE_ONLY' && stats.valid === 0) || loading}
                  className={`px-10 py-3 text-sm font-bold text-white rounded-2xl transition-all shadow-xl transform active:scale-95 flex items-center ${
                    mode === 'VALIDATE_ONLY'
                ...
                >
                  {loading ? (
                    ...
                    `Tiến hành Đồng bộ (${stats.valid} dòng)`
                  )}
                </button>
              </div>
            </>
