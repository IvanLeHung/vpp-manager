...
import {
  ...
  ArrowDownToLine, ArrowUpFromLine, Eye, X, ArrowLeftRight
} from 'lucide-react';
...
export default function Items() {
  ...
  const [transferItems, setTransferItems] = useState<{ id: string; name: string; mvpp: string; currentType: string }[]>([]);
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] p-4 md:p-8 bg-slate-50 relative overflow-hidden" onClick={() => setActiveMenuId(null)}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mb-6 shrink-0">
          ...
          <div className="lg:col-span-7 grid grid-cols-2 md:grid-cols-5 gap-3">
            {[
              ...
              ...(isEmployee ? [] : [{ label: 'Giá trị tồn', value: Number(summary.totalStockValue).toLocaleString('vi-VN') + 'đ', icon: TrendingUp, color: 'cyan' }]),
            ].map((c, i) => {
              const Icon = c.icon;
              return (
      ...
      {selectedIds.size > 0 && isAdmin && (
        <div className="absolute top-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-4 z-50">
          ...
          <button 
            onClick={() => {
              const selected = items.filter(i => selectedIds.has(i.id)).map(i => ({ id: i.id, name: i.name, mvpp: i.mvpp, currentType: i.itemType }));
              setTransferItems(selected);
              setShowTransferModal(true);
            }} 
            className="text-sm font-bold flex items-center hover:text-indigo-400 transition"
          >
            <ArrowLeftRight className="w-4 h-4 mr-1" />Chuyển kho
          </button>
                  ...
                  {loading ? (
                    ...
                    return (
                      <tr key={item.id} className={`hover:bg-indigo-50/30 transition-colors group ${!item.isActive ? 'bg-slate-50/50 opacity-75' : ''} ${isSelected ? 'bg-indigo-50/50' : ''}`}>
                        ...
                        {!isEmployee && <td className="p-4 text-right font-bold text-amber-700">{Number(item.price).toLocaleString('vi-VN')} đ</td>}
                        ...
                        <td className="p-4 text-right relative pr-6">
                          ...
                          {activeMenuId === item.id && (
                            <div className="absolute right-8 top-10 w-52 bg-white rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.1)] border border-slate-100 py-2 z-50" onClick={(e) => e.stopPropagation()}>
                              ...
                              {isAdmin && (
                                <>
                                  <button onClick={() => { setEditingItem(item); setFormData({ ...item, printSortGroup: item.printSortGroup || '' }); setShowItemForm(true); setActiveMenuId(null); }} className="w-full text-left px-4 py-2 hover:bg-slate-50 text-sm font-bold text-blue-700 flex items-center"><Edit2 className="w-4 h-4 mr-2" /> Chỉnh sửa</button>
                                  <button onClick={() => { setEditingItem(null); setFormData({ ...item, mvpp: `${item.mvpp}_COPY`, printSortGroup: item.printSortGroup || '' }); setShowItemForm(true); setActiveMenuId(null); }} className="w-full text-left px-4 py-2 hover:bg-slate-50 text-sm font-bold text-indigo-700 flex items-center"><Copy className="w-4 h-4 mr-2" /> Nhân bản</button>
                              ...
                              {isAdmin && (
                                <>
                                  ...
                                  <button 
                                    onClick={() => { 
                                      setTransferItems([{ id: item.id, name: item.name, mvpp: item.mvpp, currentType: item.itemType }]);
                                      setShowTransferModal(true);
                                      setActiveMenuId(null);
                                    }} 
                                    className="w-full text-left px-4 py-2 hover:bg-slate-50 text-sm font-bold text-indigo-600 flex items-center"
                                  >
                                    <ArrowLeftRight className="w-4 h-4 mr-2" /> 
                                    Chuyển sang kho {item.itemType === 'VPP' ? 'Vệ sinh' : 'VPP'}
                                  </button>
                                </>
                              )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSaveItem} className="p-8 overflow-y-auto bg-slate-50/50">
              ...
              <div className="mb-6">
                <label className="block text-sm font-bold text-slate-700 mb-2">Nhóm sắp xếp khi in (Tùy chọn)</label>
      ...
      <WarehouseTransferModal
        ...
        onSuccess={(msg) => {
          ...
          fetchItems();
          fetchSummary();
          refreshData(); // Refresh global AppContext items/requests
        }}
      />
    </div>
  );
