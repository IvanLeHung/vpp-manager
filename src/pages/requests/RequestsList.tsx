...
export default function RequestsList({ requests, currentUser, setViewMode, setActiveRequest, setCreateSupplyType, setNavigationIds, refreshData, showToast }: Props) {
  ...
  const itemReportRows = useMemo(() => {
    ...
    return requests.flatMap(request => {
      ...
      return (request.lines || []).flatMap((line: any) => {
        ...
        const approvedHistory = (request.approvalHistories || []).slice().reverse().find((history: any) => ['ADMIN_APPROVE', 'ADMIN_APPROVED'].includes(history.action));
  ...
  const calendarDays = useMemo(() => {
    ...
    return [
      ...Array.from({ length: firstMondayOffset }, () => null),
        ...
        <div className="flex flex-col bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden" style={{width: previewReq ? '55%' : '100%', transition: 'width 0.3s'}}>
          <div className="p-3 border-b border-slate-200 bg-slate-50/50 flex flex-col xl:flex-row gap-3 justify-between items-center">
            ...
            <div className="flex items-center gap-2 w-full xl:w-auto">
                ...
                <button onClick={() => setShowAdvancedFilters(!showAdvancedFilters)} className={`px-3 py-2 rounded-xl border font-bold text-xs flex items-center gap-1.5 ${showAdvancedFilters ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200 text-slate-600'}`}>
                   ...
                   Bộ lọc
                   {(statusFilters.length > 0 || deptFilter !== 'ALL' || priorityFilter !== 'ALL' || createdDateFilter) && (
                     <span className="min-w-5 h-5 px-1 rounded-full bg-indigo-600 text-white text-[10px] flex items-center justify-center">
                       {statusFilters.length + Number(deptFilter !== 'ALL') + Number(priorityFilter !== 'ALL') + Number(Boolean(createdDateFilter))}
                     </span>
                   )}
                </button>
                 ...
                 <div className="border-t border-slate-100 pt-3">
                   <div className="w-[320px] max-w-full rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                     ...
                     <div className="flex items-center justify-between mb-3">
                       ...
                       <span className="text-xs font-black uppercase tracking-wider text-slate-700">Tháng {calendarMonth.getMonth() + 1}/{calendarMonth.getFullYear()}</span>
                       <button onClick={() => setCalendarMonth(prev => new Date(prev.getFullYear(), prev.getMonth() + 1, 1))} className="w-7 h-7 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-500" aria-label="Tháng sau">
                         <ChevronRight className="w-4 h-4" />
                       </button>
                     </div>
                     <div className="grid grid-cols-7 gap-1 text-center mb-1">
                       {['T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'CN'].map(day => <span key={day} className={`text-[10px] font-black ${day === 'CN' ? 'text-rose-500' : 'text-slate-400'}`}>{day}</span>)}
                     </div>
          ...
          {showItemReport && (
            <div className="px-4 py-3 border-b border-indigo-100 bg-indigo-50/40">
              <div className="flex flex-wrap items-end gap-3">
                <div className="flex flex-col gap-1 min-w-[280px] flex-1"><label className="text-[9px] font-black uppercase tracking-widest text-slate-500">Vật tư cần xuất (chọn nhiều)</label><Select mode="multiple" showSearch optionFilterProp="label" allowClear value={reportItemMvpp} onChange={values => setReportItemMvpp(values as string[])} placeholder="Chọn một hoặc nhiều vật tư" options={reportItemOptions.map(item => ({ value: item.mvpp, label: `${item.name} (${item.mvpp})` }))} maxTagCount={2} popupMatchSelectWidth={360} className="w-full" /></div>
