...
export default function RequestsList({ requests, currentUser, setViewMode, setActiveRequest, refreshData, showToast }: Props) {
  ...
  const filteredRequests = useMemo(() => {
    ...
    // 'MINE' tab: show all requests created by currentUser (read-only for non-active steps)
    ...
    if (statusFilter !== 'ALL') {
      if (statusFilter === 'MY_ACTION') {
        if (currentUser.role === 'MANAGER') {
          // Strictly: only requests where I am the current approver
          filtered = filtered.filter(r => r.currentApproverId === currentUser.userId);
        } else if (currentUser.role === 'ADMIN') {
          filtered = filtered.filter(r => r.status === 'PENDING_ADMIN');
        } else if (isWarehouse) {
  ...
  const stats = useMemo(() => {
    // MY_ACTION: strictly based on currentApproverId or warehouse step
    const myActionCount = currentUser.role === 'MANAGER'
      ? requests.filter(r => r.currentApproverId === currentUser.userId).length
    ...
    // Own requests count (for any role)
    const myRequestsCount = requests.filter(r => r.requesterId === currentUser.userId).length;
  ...
  const isApprovable = (req: VPPRequest) => {
    // Strictly: only if currentApproverId === me AND request is pending at my role's step
    if (currentUser.role === 'MANAGER' && req.currentApproverId === currentUser.userId &&
        (req.status === 'PENDING_MANAGER')) return true;
    if (currentUser.role === 'ADMIN' && req.status === 'PENDING_ADMIN') return true;
    return false;
  };
          ...
          <div className="flex flex-wrap gap-2 overflow-x-auto w-full xl:w-auto">
            {isWarehouse ? (
              ...
              <>
                ...
                {/* Phiếu của tôi — visible to all roles */}
                <button onClick={() => setStatusFilter('MINE')} className={`px-4 py-2 rounded-lg font-bold text-sm whitespace-nowrap transition flex items-center gap-2 ${statusFilter==='MINE'?'bg-violet-600 text-white shadow-md':'bg-violet-50 border border-violet-200 text-violet-700 hover:bg-violet-100'}`}>
                  Phiếu của tôi
                  {stats.myRequests > 0 && (
                    <span className={`inline-flex items-center justify-center rounded-full text-[10px] font-black min-w-[18px] h-[18px] px-1 ${statusFilter === 'MINE' ? 'bg-white text-violet-600' : 'bg-violet-500 text-white'}`}>{stats.myRequests}</span>
                  )}
                </button>
                {/* Cần tôi xử lý — for managers/admins/warehouse */}
                {(currentUser.role === 'MANAGER' || currentUser.role === 'ADMIN' || isWarehouse) && (
