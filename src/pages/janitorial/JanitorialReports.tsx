...
const TEMPLATE_ITEMS = [
  { name: 'Nước lau sàn Sunlight', unit: 'Ram' },
...
const INITIAL_TICKETS: DeliveryTicket[] = [
  {
    ...
    items: [
      { name: "Nước lau sàn Sunlight", unit: "Ram", qtyRequested: 10, qtyApproved: 10, qtyReceived: 10, status: "Đã nhận đủ", note: "Nhận đủ giấy A4 bãi bằng" },
      { name: "Túi rác đen 90x120", unit: "Cái", qtyRequested: 50, qtyApproved: 50, qtyReceived: 50, status: "Đã nhận đủ", note: "Nhận đủ bút Thiên Long" },
      { name: "Nước rửa tay Lifebuoy", unit: "Cái", qtyRequested: 20, qtyApproved: 20, qtyReceived: 20, status: "Đã nhận đủ", note: "" }
    ]
  },
  {
    id: "PDX-VS-20260510-002",
    date: "2026-05-10",
  ...
  },
  {
    ...
    items: [
      { name: "Nước rửa tay Lifebuoy", unit: "Chai", qtyRequested: 10, qtyApproved: 10, qtyReceived: 10, status: "Đã nhận đủ", note: "" },
      { name: "Giấy vệ sinh cuộn lớn", unit: "Gói", qtyRequested: 100, qtyApproved: 80, qtyReceived: 80, status: "Nhận sai hàng", note: "Nhận sai mẫu khăn giấy cuộn thay vì khăn hộp" },
      { name: "Nước lau sàn Sunlight", unit: "Ram", qtyRequested: 5, qtyApproved: 5, qtyReceived: 5, status: "Đã nhận đủ", note: "" }
    ]
  }
];
...
export default function JanitorialReports() {
  ...
  const [newTicketForm, setNewTicketForm] = useState<{
    ...
    delivererSignature: '',
    receiverSignature: '',
    items: [{ name: 'Nước lau sàn Sunlight', qtyRequested: 10, qtyApproved: 10, qtyReceived: 10, note: '', isWrong: false }]
  });
  ...
  const fetchTickets = async () => {
    try {
      setLoading(true);
      const res = await api.get('/reports/vesinh-giao-nhan');
      if (res.data && Array.isArray(res.data) && res.data.length > 0) {
        setTickets(res.data);
        localStorage.setItem('vesinh_delivery_tickets', JSON.stringify(res.data));
      } else {
        // Fallback to local storage or initial templates if empty
        const saved = localStorage.getItem('vesinh_delivery_tickets');
        if (saved) {
          setTickets(JSON.parse(saved));
        } else {
          localStorage.setItem('vesinh_delivery_tickets', JSON.stringify(INITIAL_TICKETS));
          setTickets(INITIAL_TICKETS);
        }
      }
    } catch (err: any) {
      console.error('Lỗi tải báo cáo Đồ vệ sinh:', err);
      // Fallback
      const saved = localStorage.getItem('vesinh_delivery_tickets');
      if (saved) {
        setTickets(JSON.parse(saved));
  ...
  const saveTickets = (updated: DeliveryTicket[]) => {
    localStorage.setItem('vesinh_delivery_tickets', JSON.stringify(updated));
    setTickets(updated);
  };
  ...
  const handlePrint = () => {
    if (aggregatedItems.length === 0) {
      toast.warning("Chưa có dữ liệu báo cáo Đồ vệ sinh để in");
      return;
    }
  ...
  const handleExportWord = () => {
    ...
    const titleText = selectedTicket 
      ? "Báo cáo đề xuất & giao nhận đồ vệ sinh" 
      : "Báo cáo tổng hợp đồ vệ sinh";
    const qrData = selectedTicket 
      ? selectedTicket.id 
      : `BC-VS-${new Date().getFullYear()}${String(new Date().getMonth() + 1).padStart(2, '0')}${String(new Date().getDate()).padStart(2, '0')}`;
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 15px; margin-bottom: 15px;">
          <thead>
            <tr>
              ...
              <th style="border: 1px solid #000; padding: 6px; font-weight: bold; background-color: #f2f2f2; width: 100px;">Số loại vật tư</th>
    ...
    a.download = `Bao_Cao_Do_Ve_Sinh_${fileSuffix}.doc`;
  ...
  const handleAddFormItem = () => {
    setNewTicketForm({
      ...newTicketForm,
      items: [...newTicketForm.items, { name: 'Nước lau sàn Sunlight', qtyRequested: 5, qtyApproved: 5, qtyReceived: 5, note: '', isWrong: false }]
    });
  };
  ...
  const handleSaveNewTicket = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.post('/reports/vesinh-create-ticket', payload);
    ...
    <div className="w-full max-w-full min-w-0 overflow-x-hidden p-4 lg:p-6 bg-slate-50 relative flex flex-col gap-6 box-border">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4 mb-10 no-print w-full min-w-0">
        <div className="animate-in fade-in slide-in-from-left-4 duration-700">
          <div className="flex items-center gap-3 mb-1">
            ...
            <h2 className="text-3xl font-black text-slate-800 tracking-tighter uppercase italic">
              Báo cáo đề xuất & bàn giao Đồ vệ sinh
            </h2>
          </div>
        ...
        <div className="flex flex-wrap gap-3 justify-start xl:justify-end animate-in fade-in slide-in-from-right-4 duration-700 max-w-full">
          ...
          <button
            ...
            className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-[11px] uppercase tracking-widest rounded-xl shadow-lg shadow-indigo-100 transition-all flex items-center gap-2 transform active:scale-95 cursor-pointer"
          >
            <PlusCircle className="w-3.5 h-3.5" /> Tạo phiếu
          </button>

          {/* Toggle view */}
          <button
            onClick={() => setActiveView(activeView === 'report' ? 'tickets' : 'report')}
            className={`px-4 py-2.5 border-2 text-[11px] font-black uppercase tracking-widest rounded-xl shadow-sm transition-all flex items-center gap-2 transform active:scale-95 cursor-pointer ${
              activeView === 'tickets' ? 'bg-indigo-50 border-indigo-300 text-indigo-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'
            }`}
          >
            <FileText className="w-3.5 h-3.5" /> {activeView === 'report' ? 'Xem danh sách phiếu' : 'Xem bảng báo cáo'}
          </button>

          {/* Xuất file dropdown */}
          <div className="relative">
            <button
              onClick={() => { setShowExportMenu(v => !v); setShowPrintMenu(false); }}
              className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[11px] uppercase tracking-widest rounded-xl shadow-lg shadow-emerald-100 transition-all flex items-center gap-2 transform active:scale-95 cursor-pointer"
            >
              <Download className="w-3.5 h-3.5" /> Xuất file
              <svg className="w-3 h-3 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 9l-7 7-7-7"/></svg>
            </button>
            {showExportMenu && (
              <div className="absolute right-0 mt-2 w-44 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
                <button
                  onClick={() => { handleExportExcel(); setShowExportMenu(false); }}
                  className="w-full flex items-center gap-3 px-4 py-3 text-xs font-bold text-slate-700 hover:bg-emerald-50 hover:text-emerald-700 transition-colors cursor-pointer"
                >
                  <Download className="w-4 h-4 text-emerald-500" /> Xuất Excel (.xlsx)
                </button>
                <div className="h-px bg-slate-100 mx-3" />
                <button
                  onClick={() => { handleExportWord(); setShowExportMenu(false); }}
                  className="w-full flex items-center gap-3 px-4 py-3 text-xs font-bold text-slate-700 hover:bg-blue-50 hover:text-blue-700 transition-colors cursor-pointer"
                >
                  <FileText className="w-4 h-4 text-blue-500" /> Xuất Word (.docx)
                </button>
              </div>
            )}
          </div>
          ...
          <div className="relative">
            <button
              onClick={() => { setShowPrintMenu(v => !v); setShowExportMenu(false); }}
              className="px-4 py-2.5 bg-slate-700 hover:bg-slate-800 text-white font-black text-[11px] uppercase tracking-widest rounded-xl shadow-lg transition-all flex items-center gap-2 transform active:scale-95 cursor-pointer"
            >
              <Printer className="w-3.5 h-3.5" /> In báo cáo
              <svg className="w-3 h-3 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 9l-7 7-7-7"/></svg>
            </button>
            {showPrintMenu && (
              <div className="absolute right-0 mt-2 w-52 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
                <div className="px-4 pt-3 pb-2">
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Loại báo cáo</p>
                  <div className="flex gap-1.5">
                    <button
                      ...
                      className={`flex-1 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all cursor-pointer ${
                        printType === 'detail' ? 'bg-indigo-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                      }`}
                    >Chi tiết</button>
                    <button
                      onClick={() => setPrintType('department')}
                      className={`flex-1 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all cursor-pointer ${
                        printType === 'department' ? 'bg-indigo-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                      }`}
                    >Tổng hợp PB</button>
                  </div>
                </div>
                <div className="px-4 pb-2">
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Khổ giấy</p>
                  <div className="flex gap-1.5">
                    <button
                      onClick={() => setPrintOrientation('portrait')}
                      className={`flex-1 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all cursor-pointer ${
                        printOrientation === 'portrait' ? 'bg-slate-700 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                      }`}
                    >Khổ dọc</button>
                    <button
                      onClick={() => setPrintOrientation('landscape')}
                      className={`flex-1 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all cursor-pointer ${
                        printOrientation === 'landscape' ? 'bg-slate-700 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                      }`}
                    >Khổ ngang</button>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-4 py-3 text-xs font-black uppercase tracking-widest text-white bg-indigo-600 hover:bg-indigo-700 transition-colors cursor-pointer"
                >
      ...
      {activeView === 'report' ? (
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden no-print animate-in fade-in duration-300">
          <div className="p-6 border-b border-slate-100 bg-slate-50/50">
            <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest italic">Danh sách toàn bộ phiếu đề xuất & bàn giao VPP</h4>
          </div>
          <div className="table-wrapper">
            <table className="min-w-[900px] w-full text-left">
              <thead>
      ...
      <div id="print-area-wrapper" className="print-area hidden">
        <div className="report-header">
          <div className="company">
            ...
            <em>{selectedTicket ? "Báo cáo đề xuất & bàn giao VPP" : "Báo cáo tổng hợp đồ vệ sinh"}</em><br />
          ...
          <div className="qr">
            <img 
              src={`https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=${encodeURIComponent(
                selectedTicket ? selectedTicket.id : `BC-VS-${new Date().getFullYear()}${String(new Date().getMonth() + 1).padStart(2, '0')}${String(new Date().getDate()).padStart(2, '0')}`
              )}`} 
              alt="QR Code" 
            />
          </div>
        ...
        <div className="print-title">
          {selectedTicket 
            ? "BÁO CÁO ĐỀ XUẤT VÀ BÀN GIAO ĐỒ VỆ SINH" 
            : (printType === 'department' 
                ? "BÁO CÁO TỔNG HỢP SỐ LƯỢNG THEO PHÒNG BAN" 
                : "BÁO CÁO ĐỀ XUẤT VÀ BÀN GIAO ĐỒ VỆ SINH"
              )
        ...
        <div className="print-meta">
          ...
          {selectedTicket ? (
            ...
            <div><strong>Mã báo cáo:</strong> BC-VS-{new Date().getFullYear()}{String(new Date().getMonth() + 1).padStart(2, '0')}{String(new Date().getDate()).padStart(2, '0')}</div>
          )}
        </div>
